...
export class BasicSphere implements OnInit{
  ...
  ngOnInit(){
    ...
    // gui.add(params, 'animation').min(0).max(1).step(1).onFinishChange(function(){
    //     basicSphereEnv.update({animation: params.animation});
    // });

    // //adding the texture slider
    // gui.add(params, 'texture').min(0).max(7).step(1).onFinishChange(function(){
    //     basicSphereEnv.update({texture : params.texture});
    // });

    // //adding the pause button
    // gui.add(params, 'paused').onFinishChange(function(){
    //     if(params.paused)
